 ...
 * add_theme_support('smart-nav-walker');
 */

namespace Kec\Smart\Modules\Module\Nav;
...
class Walker extends \Walker_Nav_Menu {
	...
	public function __construct() {
		...
		$cpt           = get_post_type();
		$this->cpt     = \in_array( $cpt, get_post_types( [ '_builtin' => false ] ), false );
	...
	public function cssClasses( $classes, $item ) {
		...
		if ( $this->cpt ) {
			...
			if ( $this->archive && Utils::url_compare( $this->archive, $item->url ) ) {
				$classes[] = 'active';
			}
		}
...
function nav_menu_args( $args = [] ) {
	...
	if ( ! $args['walker'] ) {
		$nav_menu_args['walker'] = new Walker();
	}
